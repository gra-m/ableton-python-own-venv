...
def print_modulation_curve_params(time_list_size, amplitude_size, time_list_slice_size, amplitude_list_slice_size):
    print("MIDI_PORTS_rtmidi2_SEES", rtmidi2.get_out_ports())
    print("TIME_LIST_START/END/SPACING", TIME_LIST_START, " / ", TIME_LIST_END, " / ", SPACING)
    print("time_list_size/amplitude(list)_size", time_list_size, " / ", amplitude_size)
    print("SLICE_START/END/timeSliceSize/ampSliceSize", SLICE_START, " / ", SLICE_END, " / ", time_list_slice_size,
          " / ", amplitude_list_slice_size)
...
def send_mod(amplitude, repeat):
    ...
    for _ in range(repeat):
        if print_send_report & not_printed:
            print("scaled list put through convert range inmin/max/outmin/max", IN_MIN, "/", IN_MAX, "/", OUT_MIN, "/",
                  OUT_MAX, " of size: ", len(scaled))
            not_printed = False
        for value in scaled:
